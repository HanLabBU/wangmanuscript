function dual_sensor_vm_ssadp_trace()
    ...
    for file_idx=1:length(file_list)
        ...
        for trial_idx=1:numel(result.vm_trial)
            ...
            current_category = result.vm_trial(trial_idx).event_category.is_ssadp;
        ...
        if ~isempty(lowpass_freq)
            f_vm_trace_data = bsxfun(@minus, f_vm_trace_data, mean(f_vm_trace_data(normalize_window_idx,:),1));
        end
    ...
    title({'SS w/ ADP triggered Vm',['N=',num2str(size(plot_data,2)),' Neurons']},'Interpreter','none')
    ...
    title({'SS w/ ADP triggered GCaMP',['N=',num2str(size(plot_data,2)),' Neurons'],['One-sample t-test p=',num2str(p)]},'Interpreter','none')      
    ...
    title({'SS-w-ADP',['N=',num2str(size(plot_data,2)),' Neurons'],['R square = ',num2str(fit_result.Rsquared.Ordinary)]},'Interpreter','none') 
